...
(defn gen-runtime-diagnostics-context! [_form _env obj-sym & body]
  (debug-assert (symbol? obj-sym))
...
(defmacro oget [obj & selector]
  (let [target-obj-sym (gensym "target-obj")
    ...
    `(let [~target-obj-sym ~obj]
       ~(with-diagnostics-context! &form &env target-obj-sym
          (validate-object-statically obj)
          (gen-oget target-obj-sym selector-list)))))
...
(defmacro oset! [obj & selector+val]
  (let [target-obj-sym (gensym "target-obj")
        selector-list (butlast selector+val)
        val (last selector+val)]
    `(let [~target-obj-sym ~obj]
       ~(with-diagnostics-context! &form &env target-obj-sym
          (validate-object-statically obj)
          (gen-oset! target-obj-sym selector-list val)))))
...
(defmacro ocall [obj selector & args]
  (let [target-obj-sym (gensym "target-obj")
    ...
    `(let [~target-obj-sym ~obj]
       ~(with-diagnostics-context! &form &env target-obj-sym
          (validate-object-statically obj)
          (gen-ocall target-obj-sym selector-list args)))))
...
(defmacro oapply [obj & selector+args]
  (let [target-obj-sym (gensym "target-obj")
        selector-list (butlast selector+args)
        args (last selector+args)]
    `(let [~target-obj-sym ~obj]
       ~(with-diagnostics-context! &form &env target-obj-sym
          (validate-object-statically obj)
          (gen-oapply target-obj-sym selector-list args)))))
